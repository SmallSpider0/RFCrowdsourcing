...
class Randomizer:
    # 输入Requester的公钥
    def __init__(self, ipfs_url, provider_url, contract_address, contract_abi, requester_pk_file , id = 0):
        # 其它参数初始化
        self.id = id
    ...
    def listen_for_requests(self, is_async=False):
        ...
        self.contract_interface.listen_for_events("IntegerReceived(uint256)", self.handle_event, is_async)
    ...
    def handle_event(self, raw_event, args):
        ...
        file = pickle.loads(self.__fetch_ipfs("QmR7sAkVj6SVPL7dzCUU5bJZbPRNB18WWwK1yuDsTGFKha"))
...
if __name__=="__main__":
    ...
    # randomizer对象初始化
    randomizer = Randomizer(ipfs_url, web3_url, contract_address, contract_abi, 'tmp/keypairs/pk.pkl')
    ...
    with open("solidity/contract-abi.json") as file:
        contract_abi = json.loads(file.read()) # 智能合约ABI
    randomizer.contract_interface.send_transaction('receiveInteger', account, private_key, 123)
